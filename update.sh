...
source .scripts/checks/host_checks.sh
source .scripts/checks/docker_checks.sh
source .scripts/checks/asn_checks.sh

# Import of action scripts
source .scripts/actions/docker_tasks.sh
source .scripts/actions/updates/migration_tasks.sh
source .scripts/actions/updates/apply_new_options.sh
source .scripts/actions/ipv6_controller.sh
...
if [ ! "$DEV" ]; then
  ...
  git checkout "origin/${BRANCH}" -- .scripts
...
if [ ! "$DEV" ]; then
  ...
  echo -e "\e[33mDEVELOPER MODE: Not creating diffs or changing repository path\e[0m"
fi
