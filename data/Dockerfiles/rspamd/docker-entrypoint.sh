...
if [ "$SKIP_SPAMHAUS" == "y" ]; then
  echo -e "\e[33mSkipping Spamhaus blocklists as SKIP_SPAMHAUS is set to 'y'.\e[0m"
  cat <<EOF > /etc/rspamd/custom/disable-spamhaus-rbl.conf
# Autogenerated by mailcow due to user disable of Spamhaus RBL (SKIP_SPAMHAUS=y)
spamhaus {
  enabled = false;
}
"DBL" {
...
EOF
  ...
  rm -rf /etc/rspamd/custom/disable-spamhaus-rbl.conf
fi
